...
class env3d:
    ...
    def sense(self, x, y, r):
        ...
        for i in range(1, int(obs_num + 1)):
            ...
            yomax = hvy[4 * (i - 1) + 1] + r
            if (x >= xomin) and (x <= xomax) and (y >= yomin) and (y <= yomax):
                # if the robot is within the sensing range of the obstacle, add it to visible list
...
class RRT3d:
    ...
    # calls subroutines to find the nearest node and connect it
    def expand(self):
        # add random node
    ...
    def step(self, nnear, nrand):
        ...
        if d > dmax:
            ...
            self.add_node(nrand, x, y, z)  # this is to establish new node between rand and near
...
E = env3d(vx, vy, vz, 0, 100, 0, 100, 0, 200)
